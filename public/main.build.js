...
function map(obj, transform) {
    if (obj.then instanceof Function && !Array.isArray(obj)) {
        // doing this rather than Promise.resolve to allow non-promise output when possible
        return Promise.resolve(obj).then(function (arr) {
...
http.delete = function (endpoint, data) {
    return http('DELETE', endpoint, data);
};

var _templateObject = taggedTemplateLiteral(['<h1>Farmer John\'s Cows</h1>\n\n    ', '\n    \n    <input ', ' placeholder="new cow"\n        ', ' />\n    <button ', '>add</button>'], ['<h1>Farmer John\'s Cows</h1>\n\n    ', '\n    \n    <input ', ' placeholder="new cow"\n        ', ' />\n    <button ', '>add</button>']);
var _templateObject2 = taggedTemplateLiteral(['<p>\n            ', '\n            <button ', '>x</button>\n        </p>'], ['<p>\n            ', '\n            <button ', '>x</button>\n        </p>']);

var state = {};
re(state, 'cows');

var loadCows = function loadCows() {
    return state.cows = http.get('cows');
};
loadCows();

function deleteCow(cow) {
    var name = encodeURIComponent(cow);
    http.delete('cows/' + name).then(loadCows);
}

var newCowInput = void 0;
function addCow() {
    var name = encodeURIComponent(newCowInput.value);
    if (name) {
...
var content = bundle(_templateObject, function () {
    return map(state.cows, function (cow) {
        return bundle(_templateObject2, cow, on.click(function () {
            return deleteCow(cow);
        }));
    });
}, function (el) {
    return newCowInput = el;
}, on.keydown.enter(addCow), on.click(addCow));
